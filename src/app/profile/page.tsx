        ...
        <div className="flex flex-col items-center">
          ...
          {profile && profile.user?.image ? (
            <Image
              ...
              alt={profile.user.name || profile.user.email || "User"}
            ...
            <div className="w-24 h-24 rounded-full bg-gray-200 dark:bg-gray-700 flex items-center justify-center mb-4 text-3xl text-gray-500 dark:text-gray-300">
              {profile && profile.user?.name ? profile.user.name[0] : profile && profile.user?.email ? profile.user.email[0] : "U"}
            </div>
          )}
          {/* Name and email */}
          <h1 className="text-2xl font-bold mb-2 text-gray-800 dark:text-gray-100">
            {profile && profile.user?.name ? profile.user.name : "No Name"}
          </h1>
          <p className="text-gray-600 dark:text-gray-300 mb-1">{profile && profile.user?.email ? profile.user.email : "No Email"}</p>
          {profile && profile.user?.id && (
            <p className="text-xs text-gray-400 dark:text-gray-500 mb-4">User ID: {profile.user.id}</p>
          )}
        </div>
        <div className="mt-6">
          <h2 className="text-lg font-semibold text-gray-700 dark:text-gray-200 mb-2">Profile Details</h2>
          <ul className="text-gray-600 dark:text-gray-300 text-sm space-y-1">
            Credits: {profile && profile.user?.creditBalance ? profile.user.creditBalance : "No Credits"}
          </ul>
        </div>
      </div>
    </div>
